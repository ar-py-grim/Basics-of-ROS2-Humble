...
from my_robot_interfaces.action import RobotPos
...
class MoveRobotServerNode(Node):
    ...
    def goal_callback(self, goal_request: RobotPos.Goal):
    ...
    def execute_callback(self, goal_handle: ServerGoalHandle):
        ...
        result = RobotPos.Result()
        feedback = RobotPos.Feedback()
        ...
        while rclpy.ok():
            ...
            if goal_handle.is_cancel_requested:
                ...
                if goal_position == self.robot_position_:
                    result.message = "Successfully reached even after cancel request"
                    goal_handle.succeed()
            ...
            elif diff > 0:           # robot forward motion
                if diff >= velocity:
                    self.robot_position_ += velocity
                else:
                    self.robot_position_ += diff

            else:                    # robot backward motion
                if abs(diff) >= velocity:
                    self.robot_position_ -= velocity
                else:
                    self.robot_position_ -= abs(diff)

            self.get_logger().info("Robot position: " + str(self.robot_position_)+"m")
